...
// Input: x     horizontal position of the start of the line, columns from the
// left edge
...
void Output_Init(void) {
  ...
  ST7735_SetRotation(1); // sets screen to lanscape
  ST7735_FillScreen(0);  // set screen to black
}
...
uint8_t ST7735_PrintPerson(uint32_t i, Person_t *person, uint8_t isTop) {
  if (isTop) {
    i = i + 256;
  }
  // print person walking or standing
  if (!person->walking) {
    if (person->stand[person->cDir][i] != 0xffff) {
      writedata((uint8_t)(person->stand[person->cDir][i] >> 8));
      writedata((uint8_t)person->stand[person->cDir][i]);
      return 1;
    }
  } else {
    if (person->walk[person->cDir][person->cLeg][i] != 0xffff) {
      writedata((uint8_t)(person->walk[person->cDir][person->cLeg][i] >> 8));
      writedata((uint8_t)person->walk[person->cDir][person->cLeg][i]);
      person->cLeg ^= 1;
      return 1;
    }
  }
...
void ST7735_PrintMap(Map_t *Map, const uint16_t *trainer, int32_t row,
                     int32_t col, int upDown, int leftRight, int adjust) {
  int r, j, c, i;
  int x = -1;
  int y = 0;
  int pR = 31;
  ...
  for (r = startRow; r < endRow; r++) {
    ...
    // iterate through pxMap of tile by row
    while (j > -1) {
      x = -1; // reset x coor tracker
      // skip rows at the end
      if (r == endRow - 1) {
        // adjust row skip based on move direction
        if (upDown != 0) {
          if (upDown == -1) {
            if (j <= -1 - ((16 - adjust) * (upDown))) {
              break;
            }
          } else {
            if (j <= 15 - ((16 - adjust))) {
          ...
          if (j <= 15 - ((16 - adjust) * (upDown))) {
            break;
          }
        }
      }
      // iterate through tiles by col
      for (c = startCol; c <= endCol; c++) {
        // set personR and personC if not already
        // skip px at beginning
        if (c == startCol) {
          if (leftRight == 1) {
            i = ((j + 1) * 16) - (adjust * leftRight);
          } else {
            i = (j * 16) - (adjust * leftRight);
          }
        } else {
          i = (j * 16);
        }
        // iterate through pxMap by px
        while (i < (j + 1) * 16) {
          x++; // keep track of x coor
          // skip px at end
          if (c == endCol && leftRight != 0) {
            if (leftRight == 1) {
              if (i >= (j * 16) - ((16 - adjust) * (-leftRight))) {
                break;
              }
            } else if (leftRight == -1) {
              if (i >= ((j + 1) * 16) - ((16 - adjust) * (-leftRight))) {
                break;
              }
            }
          } else if (c == endCol) {
          ...
          // either print map px or user sprite px based on xy coor
          if ((x >= 74 && x < 106) && (y >= 67 && y < 99)) {
            pC++;
            if (pC == 32) {
              pC = 0;
              pR--;
            }
            if (Map->tiles[r][c]->hasLink &&
                Map->tiles[r][c]->linkTile->has_person &&
                ST7735_PrintPerson(i, Map->tiles[r][c]->linkTile->person,
                                   1)) { // print linked person
            } else if (Map->tiles[r][c]->hasOverlay && !Map->tiles[r][c]->isFlower && !Map->tiles[r][c]->occupied &&
                       Map->tiles[r][c]->overlay[i] !=
                           0xffff) { // print overlay and person on tile
              writedata((uint8_t)(Map->tiles[r][c]->overlay[i] >> 8));
              writedata((uint8_t)Map->tiles[r][c]->overlay[i]);
						} else if (trainer[(pR * 32) + pC] != 0xffff) { // print trainer
              writedata((uint8_t)(trainer[(pR * 32) + pC] >> 8));
              writedata((uint8_t)trainer[(pR * 32) + pC]);
            } else if (Map->tiles[r][c]->has_person &&
                       ST7735_PrintPerson(i, Map->tiles[r][c]->person,
                                          0)) { // print person on tile

            } else if (Map->tiles[r][c]->hasOverlay &&
                       Map->tiles[r][c]->overlay[i] !=
                           0xffff) { // print overlay and person on tile
              writedata((uint8_t)(Map->tiles[r][c]->overlay[i] >> 8));
              writedata((uint8_t)Map->tiles[r][c]->overlay[i]);
						} else {
              writedata((uint8_t)(Map->tiles[r][c]->pxMap[i] >>
                                  8)); // send the top 8 bits
              writedata((uint8_t)Map->tiles[r][c]
                            ->pxMap[i]); // send the bottom 8 bits
            }
          } else {
            if (Map->tiles[r][c]->hasOverlay &&
                Map->tiles[r][c]->overlay[i] != 0xffff) {
              writedata((uint8_t)(Map->tiles[r][c]->overlay[i] >> 8));
              writedata((uint8_t)Map->tiles[r][c]->overlay[i]);
            } else if (Map->tiles[r][c]->hasLink &&
                       Map->tiles[r][c]->linkTile->has_person &&
                       ST7735_PrintPerson(i, Map->tiles[r][c]->linkTile->person,
                                          1)) { // print linked person
            } else if (Map->tiles[r][c]->has_person &&
...
void ST7735_DrawBitmapT(int16_t x, int16_t y, const uint16_t *image, int16_t w,
                        int16_t h) {
  int16_t skipC = 0; // non-zero if columns need to be skipped due to clipping
  int16_t originalWidth = w; // save this value; even if not all columns fit on
                             // the screen, the image is still this width in ROM
  ...
  for (y = 0; y < h; y = y + 1) {
    for (x = 0; x < w; x = x + 1) {
      ...
      if (image[i] != 0xFFFF) {
        ST7735_DrawPixel(xcoord, ycoord, image[i]);
      }
      xcoord++;
      i = i + 1;
    }
